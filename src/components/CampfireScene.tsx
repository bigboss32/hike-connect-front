 ...
 * Supports scroll-based parallax via scrollY prop.
 */
...
const CampfireScene = ({ scrollY = 0 }: Props) => {
  // Clamp parallax to avoid over-scroll artifacts
  ...
  return (
    <div className="relative w-full overflow-hidden rounded-b-2xl" style={{ height: 220 }}>
      ...
      <div
        className="absolute inset-0 bg-gradient-to-b from-orange-300/40 via-amber-200/25 to-background dark:from-indigo-950/40 dark:via-slate-900/30 dark:to-background"
      ...
      <svg viewBox="0 0 400 160" preserveAspectRatio="xMidYMax slice"
        ...
        {/* ── Stars (parallax: fastest, farthest) ── */}
        <g style={{ transform: `translateY(${s * -0.12}px)` }}>
          {[
            ...
            <circle key={i} cx={st.x} cy={st.y} r={0.6 + (i % 3) * 0.3} fill="white" opacity="0.25"
        ...
        {/* ── Tent ── */}
        <g opacity="0.2" style={{ transform: `translateY(${s * -0.02}px)` }}>
