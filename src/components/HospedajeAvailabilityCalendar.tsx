...
import { useHospedajeAvailability, type HospedajeDayAvailability } from "@/hooks/useHospedajeAvailability";
...
const HospedajeAvailabilityCalendar = ({ hospedajeId, selectedDate, onSelectDate }: Props) => {
  const now = new Date();
  const [viewMonth, setViewMonth] = useState(now.getMonth() + 1);
  const [viewYear, setViewYear] = useState(now.getFullYear());

  const { data: availability, isLoading } = useHospedajeAvailability(hospedajeId, viewMonth, viewYear);

  const dayMap = useMemo(() => {
    const map = new Map<string, HospedajeDayAvailability>();
    availability?.dates.forEach((d) => map.set(d.date, d));
    return map;
  }, [availability]);
  ...
  const getSlotColor = (day: HospedajeDayAvailability) => {
    if (!day.is_available) return "bg-destructive/15 text-destructive";
    const ratio = day.available_guests / 20; // approximate
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <Button variant="ghost" size="icon" className="h-8 w-8" onClick={goToPrevMonth} disabled={isPrevDisabled}>
      ...
      <div className="grid grid-cols-7 gap-1">
        {WEEKDAY_LABELS.map((label) => (
          <div key={label} className="text-center text-xs font-medium text-muted-foreground py-1">{label}</div>
        ))}
      </div>
        ...
        <div className="grid grid-cols-7 gap-1">
          {calendarDays.map((dayNum, i) => {
            ...
            return (
              ...
              >
                <span className={cn("font-medium leading-none", isSelected && "font-bold")}>{dayNum}</span>
                {dayData && !isPast && (
                  <span className={cn(
                    "text-[9px] font-semibold leading-none rounded-full px-1",
                    isSelected ? "text-primary-foreground/80" : getSlotColor(dayData)
                  )}>
                    {dayData.is_available ? dayData.available_guests : "Lleno"}
                  </span>
                )}
              </button>
            );
      ...
      {selectedDate && dayMap.get(selectedDate) && (
        <div className="p-3 rounded-xl bg-primary/5 border border-primary/20 text-center">
          ...
          <p className="text-sm font-semibold text-primary mt-0.5">
            {dayMap.get(selectedDate)!.available_guests} cupos disponibles
          </p>
        </div>
      )}
    </div>
...
export default HospedajeAvailabilityCalendar;
