/**
 * RoutesHeroScene — Polished animated SVG landscape with ❤️
 * "rutas" = trail with hiker, backpack, wildlife • "hospedajes" = cabin with garden, campfire, cat
  ...
  return (
    <div className="absolute inset-0 overflow-hidden z-0">
      ...
      <svg viewBox="0 0 400 200" preserveAspectRatio="xMidYMax slice"
        className="absolute inset-0 w-full h-full rs-scene" aria-hidden="true">
        <defs>
        ...
        {isNight ? (
          <g className="rs-celestial">
            ...
            <circle cx="318" cy="30" r="0.8" fill="#CBD5E1" opacity="0.15" />
            {/* Stars — varied sizes */}
            {[{x:22,y:12,r:1.2},{x:65,y:22,r:0.8},{x:105,y:8,r:1.4},{x:148,y:28,r:0.9},
              ...
              <circle key={i} cx={s.x} cy={s.y} r={s.r} fill="white" className="rs-twinkle" style={{ animationDelay: `${i * 0.35}s` }} />
            ))}
            {/* Shooting star */}
            <line x1="80" y1="18" x2="100" y2="25" stroke="white" strokeWidth="0.6" opacity="0.3" className="rs-shootingStar" />
          </g>
        ) : (
          <g className="rs-sun-group">
            <circle cx="320" cy={isDawn ? 55 : isSunset ? 48 : 26} r="24" fill="url(#rs-sun)" />
            <circle cx="320" cy={isDawn ? 55 : isSunset ? 48 : 26} r="8" fill={sunColor} opacity="0.85" />
            {/* Sun rays */}
            {[0,60,120,180,240,300].map(a => (
              <line key={a} x1="320" y1={isDawn ? 55 : isSunset ? 48 : 26}
                x2={320 + Math.cos(a * Math.PI/180) * 16} y2={(isDawn ? 55 : isSunset ? 48 : 26) + Math.sin(a * Math.PI/180) * 16}
                stroke={sunColor} strokeWidth="0.5" opacity="0.2" className="rs-ray" />
            ))}
          </g>
        )}

        {/* ── CLOUDS (3 layers for depth) ── */}
        ...
        {/* ── WATERFALL on near mountain ── */}
        <path d="M172,112 L172,130" fill="none" stroke="hsl(200,50%,60%)" strokeWidth="1" opacity="0.2" className="rs-waterfall" />
        <ellipse cx="172" cy="132" rx="3" ry="1" fill="hsl(200,50%,60%)" opacity="0.1" />

        {/* ── River/stream winding through ── */}
        ...
        <g className="rs-mode-layer" style={{ opacity: isHospedaje ? 0 : 1, transform: isHospedaje ? 'translateY(6px) scale(0.97)' : 'translateY(0) scale(1)' }}>
          ...
          {/* Hiker with backpack & walking stick */}
        ...
        <g className="rs-mode-layer" style={{ opacity: isHospedaje ? 1 : 0, transform: isHospedaje ? 'translateY(0) scale(1)' : 'translateY(6px) scale(0.97)' }}>
          ...
          {isHospedaje && <>
            <circle cx="222" cy="104" r="2.5" fill="currentColor" className="text-muted-foreground rs-smoke1" />
            <circle cx="224" cy="97" r="2" fill="currentColor" className="text-muted-foreground rs-smoke2" />
            <circle cx="221" cy="91" r="3" fill="currentColor" className="text-muted-foreground rs-smoke3" />
            <circle cx="223" cy="85" r="2.2" fill="currentColor" className="text-muted-foreground rs-smoke4" />
          </>}
          ...
          {/* Campfire beside cabin */}
          <g className="rs-campfireGroup">
          ...
          <g>
            {[{x:85,y:130,c:"#F59E0B"},{x:280,y:125,c:"#A78BFA"}].map((bf,i) => (
              <g key={i} className="rs-butterfly" style={{ animationDelay: `${i * 2}s` }}>
                <ellipse cx={bf.x - 2} cy={bf.y} rx="1.5" ry="2" fill={bf.c} opacity="0.35">
                  <animate attributeName="rx" values="1.5;0.5;1.5" dur="0.3s" repeatCount="indefinite" />
                </ellipse>
                <ellipse cx={bf.x + 2} cy={bf.y} rx="1.5" ry="2" fill={bf.c} opacity="0.35">
                  <animate attributeName="rx" values="1.5;0.5;1.5" dur="0.3s" repeatCount="indefinite" />
                </ellipse>
          ...
          <g>
            {[{x:60,d:0},{x:170,d:3},{x:300,d:6}].map((l,i) => (
              <ellipse key={i} cx={l.x} cy="0" rx="2" ry="1" fill="hsl(35,50%,45%)" opacity="0.2" className="rs-leaf"
                style={{ animationDelay: `${l.d}s` }} transform={`rotate(${30 + i*20} ${l.x} 0)`} />
            ))}
          </g>
        )}
      </svg>
      ...
      <style>{`
        ...
        .rs-mode-layer { transition: opacity 0.6s cubic-bezier(0.4,0,0.2,1), transform 0.6s cubic-bezier(0.4,0,0.2,1); }
        ...
        .rs-smoke4 { opacity: 0.02; animation: rs-smoke 4s ease-out 2s infinite; }
        ...
        .rs-flame1 { animation: rs-flicker 0.8s ease-in-out infinite; }
        .rs-flame2 { animation: rs-flicker 0.6s ease-in-out 0.2s infinite; }
        .rs-flame3 { animation: rs-flicker 0.7s ease-in-out 0.4s infinite; }
        .rs-spark { animation: rs-sparkRise 2s ease-out infinite; }
        .rs-cat { animation: rs-catBreath 3s ease-in-out infinite; }
        ...
        @keyframes rs-flicker { 0%,100% { transform: scaleY(1) scaleX(1); opacity: inherit; } 50% { transform: scaleY(1.15) scaleX(0.9); opacity: 0.7; } }
        ...
        @keyframes rs-waterfallFlow { 0% { stroke-dashoffset: 0; } 100% { stroke-dashoffset: -6; } }
