...
const StatCard = ({ icon, value, label, color, delay, onClick, className = "" }: StatCardProps) => (
  <div 
    className={`backdrop-blur-sm bg-foreground/[0.04] border border-foreground/[0.06] rounded-2xl p-4 text-center transition-all duration-200 ${onClick ? "cursor-pointer hover:bg-foreground/[0.08] hover:-translate-y-0.5" : ""} ${className}`}
...
const UserStatsCards = ({ hideGreeting = false }: UserStatsCardsProps) => {
  ...
  return (
    <Card className="backdrop-blur-md bg-card/75 border-border/50 shadow-soft">
